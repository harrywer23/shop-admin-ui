    ...
    >
      <template v-slot:body-cell-actions="props">
        <q-td>
          ...
          <q-btn color="info" @click="viewReceivedCoupons(props.row.couponId)">查看已领取</q-btn>
        </q-td>
      </template>
    </q-table>
    ...
    <q-dialog v-model="dialog" persistent>
      <q-card>
        ...
        <q-card-actions>
          ...
          <q-btn @click="dialog = false">取消</q-btn>
        </q-card-actions>
      </q-card>
    </q-dialog>
  </div>
...
async function fetchCoupons() {
  const response = await api.get('/sys/coupon/list');
...
async function saveCoupon() {
  ...
  if (isEdit.value) {
    await api.post('/sys/coupon/update', form.value);
  } else {
    await api.post('/sys/coupon/add', form.value);
  }
  dialog.value = false;
  fetchCoupons();
}
...
async function deleteCoupon(couponId) {
  await api.delete(`/sys/coupon/del/${couponId}`);
  fetchCoupons();
}

async function toggleStatus(coupon) {
  const newStatus = coupon.status === 1 ? 0 : 1; // 切换状态
  await api.post('/sys/coupon/update', { ...coupon, status: newStatus });
  fetchCoupons();
}

function viewReceivedCoupons(couponId) {
  router.push({ path: `/admin/coupon/received`, query: { couponId } });
}
